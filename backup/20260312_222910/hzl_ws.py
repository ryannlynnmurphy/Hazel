...
try:
    from smarthome import turn_on, turn_off
    HA_AVAILABLE = True
except ImportError:
    logging.warning("smarthome.py not found — home control disabled")
    HA_AVAILABLE = False
...
async def handle_action(ws: WebSocketServerProtocol, data: dict):
    ...
    elif action in ("turn_on", "turn_off") and entity_id:
        if HA_AVAILABLE:
            try:
                if action == "turn_on":
                    await asyncio.to_thread(turn_on, entity_id)
                else:
                    await asyncio.to_thread(turn_off, entity_id)
                log.info(f"HA: {action} {entity_id}")
            except Exception as e:
                log.error(f"Home Assistant error: {e}")
        else:
            log.warning(f"HA action received but smarthome.py unavailable")
